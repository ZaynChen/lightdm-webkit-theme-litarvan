...
import LLogin from "@/pages/base/login.vue";
import { onMounted } from "vue";
import { settings } from "@/ts/settings";
import { trans } from "@/ts/translations";

const fix = greeter !== undefined;
  ...
  <div id="setup" :class="{ fix: fix }" v-italic>
    ...
    <div id="left-settings" class="settings">
      <div class="checkbox-line">
        <LCheckbox v-model="settings.disableSplash" /><label>{{
          texts.disableSplash
        }}</label>
      </div>
      <div class="checkbox-line">
        <LCheckbox v-model="settings.disableSplashText" /><label>{{
          texts.disableSplashText
        }}</label>
      </div>
      <div class="checkbox-line">
        <LCheckbox v-model="settings.disableIntro" /><label>{{
          texts.disableIntro
        }}</label>
      </div>
      <div class="checkbox-line">
        <LCheckbox v-model="settings.clock12" /><label>{{
          texts.clock12
        }}</label>
      </div>
      <div class="checkbox-line">
        <LCheckbox v-model="settings.disablePowerTexts" /><label>{{
    ...
    <div id="right-settings" class="settings">
      <div class="disable-zoom">
        -
        <div class="checkbox-line">
          <LCheckbox v-model="settings.disableZoom" /><label>{{
            texts.disableZoom
          }}</label>
        </div>
      </div>
      <div class="checkbox-line">
        <LCheckbox v-model="settings.disableFade" /><label>{{
          texts.disableFade
        }}</label>
      </div>
      <div class="checkbox-line">
        <LCheckbox v-model="settings.roundAvatar" /><label>{{
          texts.roundAvatar
        }}</label>
      </div>
      <div class="checkbox-line">
        <LCheckbox v-model="settings.disableAvatar" /><label>{{
          texts.disableAvatar
        }}</label>
      </div>
      <div class="checkbox-line">
        <LCheckbox v-model="settings.hideUsername" /><label>{{
          texts.hideUsername
        }}</label>
      </div>
      <div class="checkbox-line">
...
.layout {
  ...
  border: solid 2px lch(from var(--outer-foreground) calc(l - 60) c h);
  border-radius: 2px;
...
.layout:hover {
  cursor: pointer;
  border-color: lch(from var(--outer-foreground) calc(l - 30) c h);
}

.layout.selected {
  border-color: var(--outer-foreground);
}
