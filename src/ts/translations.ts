...
function getLocale(): string {
  // using user locale
  if (lastUser.value?.language && lastUser.value.language !== "") {
    return lastUser.value.language.replace(/\.UTF-*8/i, "").replace("_", "-");
  }
...
function italicDisabled() {
  const disabled = ["zh-CN", "zh-TW"];
