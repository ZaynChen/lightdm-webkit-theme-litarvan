...
const battery = reactive({
  ...
  value: 15,
  isCharging: false,
  // canAccess: greeter?.can_access_battery ?? false,
  canAccess: false,
});
...
const brightness = reactive({
  ...
  value: 85,
  // canAccess: greeter?.can_access_brightness ?? false,
  canAccess: false,
  showSlider: false,
});
...
watch(
  ...
  (val, oldValue) => {
    ...
    // greeter?.brightness = val;
  },
);

// function on_battery_update() {
//   // const batteryData = greeter?.battery_data ?? null;
//   const batteryData = null;
//   if (batteryData == null) {
...
//   // const bright = greeter?.brightness ?? null;
//   const bright = null;
//   if (bright == null) {
...
onMounted(() => {
  // if (greeter?.can_access_battery) {
  //   on_battery_update();
  //   greeter?.battery_update.connect(on_battery_update);
  // }
  // if (greeter?.can_access_brightness) {
  //   on_brightness_update();
  //   greeter?.brightness_update.connect(on_brightness_update);
  // }
});

onBeforeUnmount(() => {
  // if (greeter?.can_access_battery) {
  //   greeter?.battery_update.disconnect(on_battery_update);
  // }
  // if (greeter?.can_access_brightness) {
  //   greeter?.brightness_update.disconnect(on_brightness_update);
  // }
});
  ...
  <div id="additions" :class="{ small: small }">
    ...
    >
      ...
      <img
        ...
        class="addition-icon"
        :src="getImageUrl('../assets/images/battery/bolt.svg')"
        v-if="battery.isCharging"
      />
      <span id="battery-value"> {{ battery.value }}% </span>
    </div>

    <div
      id="brightness-container"
      class="addition-container"
      v-if="brightness.canAccess"
      @mouseover="brightness.showSlider = !preview ? true : false"
      @mouseleave="brightness.showSlider = false"
    >
      <img
        id="brightness-icon"
        class="addition-icon"
      ...
      <span id="brightness-value"> {{ brightness.value }}% </span>
    </div>
  </div>
</template>
